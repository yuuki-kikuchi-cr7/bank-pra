...
func TestGetAccountAPI(t *testing.T) {
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			//ルーターの処理結果(ステータスコード、レスポンスボディ)を記録する。
			...
			//仮のリクエストをルーターに渡してserver.getAccountが呼ばれる。
			//server.getAccount内でstore.store.GetAccountがモックとして呼び出される。
			//モックが設定した値(account)を返し、それがレスポンスに記録される。
...
func TestDeleteAccountAPI(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().DeleteAccounts(gomock.Any(), gomock.Eq(account.ID)).Times(1).Return(nil)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusOK, recorder.Code)
				expectedBody := fmt.Sprintf(`{"message":"account with id: %d was deleted successfully"}`, account.ID)
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().DeleteAccounts(gomock.Any(), gomock.Eq(account.ID)).Times(1).Return(sql.ErrConnDone)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusInternalServerError, recorder.Code)
			},
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().DeleteAccounts(gomock.Any(), gomock.Any()).Times(0)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusBadRequest, recorder.Code)
			},
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().DeleteAccounts(gomock.Any(), gomock.Eq(account.ID)).Times(1).Return(sql.ErrNoRows)
	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().UpdateAccounts(gomock.Any(), gomock.Eq(arg)).Times(1).Return(account, nil)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusOK, recorder.Code)
				requireBodyMatchAccount(t, recorder.Body, account)
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().UpdateAccounts(gomock.Any(), gomock.Any()).Times(0)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusBadRequest, recorder.Code)
			},
